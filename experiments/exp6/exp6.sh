...
do
    ...
    time $BIN -s byp -f "$INPUT/$LINE" -o "$OUT/p0/" --relax --heur-root 0 --feas-root 0 --feas-nodes 0 --inherit-cols 0 --pricing-method 0 --pricing-exact-time $TIME_PRI --time $TIME
    # Pricing #0: exact
    time $BIN -s byp -f "$INPUT/$LINE" -o "$OUT/p1/" --relax --heur-root $HEUR_ROOT --heur-2step-variant $VARIANT --heur-semigreedy-iter $REPETITIONS_HEUR --heur-semigreedy-alpha $ALPHA_HEUR --feas-root 0 --feas-nodes 0 --inherit-cols 0 --pricing-method 0 --pricing-exact-time $TIME_PRI --time $TIME
