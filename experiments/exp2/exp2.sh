...
do
    ...
    do
        if [[ "$h" == "greedy1s" ]]; then
            ...
            time $BIN -s heur -f "$INPUT/$LINE" -o "$OUT/$h/" --heur-initial 1 --heur-semigreedy-alpha 0.15 --preproc-off
        elif [[ "$h" == "greedy2s" ]]; then
            for v in "${VARIANTS[@]}"
            do
                echo "Solving with heuristic: $h, variant: $v"
                time $BIN -s heur -f "$INPUT/$LINE" -o "$OUT/$h/v$v/" --heur-initial 2 --heur-2step-variant $v --heur-semigreedy-alpha 0.15 --preproc-off
            done
        elif [[ "$h" == "semigreedy2s" ]]; then
            for v in "${VARIANTS[@]}"
            do
                ...
                do
                    ...
                    time $BIN -s heur -f "$INPUT/$LINE" -o "$OUT/$h/v$v/r$r/" --heur-initial 3 --heur-2step-variant $v --heur-semigreedy-iter $r --heur-semigreedy-alpha 0.15 --preproc-off
                done
            done
        fi
    done
